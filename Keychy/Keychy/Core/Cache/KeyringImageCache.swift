...
class KeyringImageCache {
    ...
    /// 위젯 선택지에서 키링 제거 (앱 캐시는 유지)
    /// - removeKeyring()과 달리 썸네일은 삭제하지 않음
    /// - 위젯용 이미지와 메타데이터만 제거
    func removeFromWidget(id: String) {
        // 위젯용 이미지만 삭제 (썸네일은 유지)
    ...
    /// 위젯에 추가되어 있는지 확인
    func isAddedToWidget(id: String) -> Bool {
        loadWidgetKeyrings().contains { $0.id == id }
    }
